 ...
 *
 * 2) Or (set ADMIN_PROJECT_ID, ADMIN_CLIENT_EMAIL, ADMIN_PRIVATE_KEY in env):
 *    ADMIN_PROJECT_ID="your-project-id" \
 *    ADMIN_CLIENT_EMAIL="service-account@your-project.iam.gserviceaccount.com" \
 *    ADMIN_PRIVATE_KEY="$(cat ./keys/sa.key | sed ':a;N;$!ba;s/\n/\\n/g')" \
...
type Env = {
  ...
  NEXT_PUBLIC_ADMIN_PROJECT_ID?: string;
...
function initAdmin() {
  ...
  // Option B: Initialize from env vars ADMIN_PROJECT_ID, ADMIN_CLIENT_EMAIL, ADMIN_PRIVATE_KEY
  if (env.ADMIN_PROJECT_ID && env.ADMIN_CLIENT_EMAIL && env.ADMIN_PRIVATE_KEY) {
    // Private key in env may have \n literal sequences — convert them to actual newlines
    const privateKey = env.ADMIN_PRIVATE_KEY.replace(/\\n/g, "\n");
  ...
  console.error("No admin credentials found. Set GOOGLE_APPLICATION_CREDENTIALS, SERVICE_ACCOUNT_PATH, or ADMIN_PROJECT_ID / ADMIN_CLIENT_EMAIL / ADMIN_PRIVATE_KEY in env.");
  process.exit(1);
}
