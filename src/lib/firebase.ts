...
async function resolveFinalStockDocIdForProduct(
  ...
  if (!pidSnapshot.empty) {
    const docSnap = pidSnapshot.docs[0];
    return docSnap.id;
  }
...
/**
 * Delete a batch from a specific stage only.
 * - Removes the stage from selectedProcesses.
 * - Resets the processingStages payload for that stage.
 * - Deletes the batch document only if no stages remain.
 */
export async function deleteBatchFromStage(
  id: string,
  stage: ProcessingStageName,
): Promise<void> {
  let batchRef = doc(db, BATCHES_COLLECTION, id);
  ...
  const batch = { id: docSnapshot.id, ...(docSnapshot.data() as any) } as Batch;
  const selectedProcesses = Array.isArray(batch.selectedProcesses)
    ? [...batch.selectedProcesses]
    : [];

  if (!selectedProcesses.includes(stage)) {
    const alreadyDeletedError = new Error("Batch is already deleted from this stage");
    (alreadyDeletedError as any).code = "already-deleted";
    throw alreadyDeletedError;
  }

  const remainingStages = selectedProcesses.filter((s) => s !== stage);
  ...
  const updates: Record<string, any> = {
    selectedProcesses: remainingStages,
  ...
  const allRemainingCompleted = remainingStages.every(
    (s) => Boolean(batch.processingStages?.[s]?.completed),
  );
  updates.status = allRemainingCompleted ? "Completed" : "In Progress";
...
interface ExecuteStageTransactionParams {
  batchId: string;
  stage: ProcessingStageName;
  acceptedQty: number;
  rejectedQty: number;
...
/**
 * Execute a single stage for a batch inside a Firestore transaction.
 *
 * This function is the canonical path for stage execution writes. It guarantees
 * atomicity between:
 * - Produced quantity updates (accepted/rejected counters on the batch stage)
 * - Raw material / intermediate consumption derived from batch.materials
 * - Produced stock increments (Store or Final Stock, depending on stage)
 */
export async function executeStageTransaction(
  params: ExecuteStageTransactionParams,
): Promise<void> {
  const { batchId, stage, acceptedQty, rejectedQty, operatorId, timestamp } =
    params;

  if (!Number.isFinite(acceptedQty) || acceptedQty <= 0) {
    throw new Error("Invalid accepted quantity");
  }
  if (!Number.isFinite(rejectedQty) || rejectedQty < 0) {
    throw new Error("Invalid rejected quantity");
  }

  const batchRef = doc(db, BATCHES_COLLECTION, batchId);

  // Pre-read batch once to resolve product mapping for Final Stock / Store.
  const preBatchSnap = await getDoc(batchRef);
  if (!preBatchSnap.exists()) {
    throw new Error(`Batch ${batchId} not found`);
  }
  const preBatchData = preBatchSnap.data() as any;
  const preBatchProductId: string | undefined = preBatchData.productId;

  let resolvedProductDocId: string | null = null;
  if (preBatchProductId) {
    resolvedProductDocId = await resolveFinalStockDocIdForProduct(
      preBatchProductId,
    );
  }
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const stageState: any = batch.processingStages?.[stage];
    if (!stageState) {
      throw new Error("Batch stage not configured");
    }
    if (stageState.completed) {
      throw new Error("Batch stage already finalized");
    }

    const materialsForStage = (batch.materials || []).filter(
      (m: any) => m.stage === stage && Number(m.quantity) > 0,
    );

    if (materialsForStage.length === 0) {
      throw new Error("Raw material input missing for stage");
    }

    const totalStageInput = materialsForStage.reduce(
      (sum: number, m: any) => sum + Number(m.quantity || 0),
      0,
    );

    if (!Number.isFinite(totalStageInput) || totalStageInput <= 0) {
      throw new Error("Raw material input missing for stage");
    }

    // 1) Compute consumption per material, proportional to its share of stage input.
    const consumptionByMaterial: Record<string, number> = {};
    for (const material of materialsForStage) {
      const baseQty = Number(material.quantity || 0);
      if (!Number.isFinite(baseQty) || baseQty <= 0) continue;
      const proportion = baseQty / totalStageInput;
      const consumed = proportion * acceptedQty;
      const normalized = Number(consumed.toFixed(4));
      if (normalized > 0) {
        consumptionByMaterial[material.id] = normalized;
      }
    }

    // 2) Apply raw/intermediate consumption against inventory documents.
    for (const material of materialsForStage) {
      const consumedQty = consumptionByMaterial[material.id];
      if (!consumedQty || consumedQty <= 0) continue;

      const rawRef = doc(db, "rawMaterials", material.id);
      ...
      if (rawSnap.exists()) {
        ...
        const finRef = doc(db, "finalStock", material.id);
        ...
        if (finSnap.exists()) {
          ...
          throw new Error(
            `Inventory document not found for material ${material.id}`,
          );
        }
      }
    }

    // 3) Produced stock update (stage-specific).
    if (resolvedProductDocId) {
      const productRef = doc(db, "finalStock", resolvedProductDocId);
      const productSnap = await transaction.get(productRef);

      if (!productSnap.exists()) {
        throw new Error(
          `Final stock product ${resolvedProductDocId} not found for batch`,
        );
      }
      ...
      if (stage === "Testing") {
        // Final stage → increment finished goods quantity and append batch entry.
        ...
        const batchEntry = {
          ...
          sourceBatchId: batch.batchId || batch.id,
          quantity: acceptedQty,
          sku: productData.sku || `BATCH-${batch.batchId || batch.id}`,
          createdAt: isoTimestamp,
        };

        transaction.update(productRef, {
          batches: arrayUnion(batchEntry),
        });
      } else {
        // Intermediate stages → increment Store quantities when linked stage
        // material IDs are available on the product.
        let stageMaterialId: string | undefined;
        if (stage === "Molding") {
          stageMaterialId = productData.mouldedMaterialId;
        } else if (stage === "Machining") {
          stageMaterialId = productData.machinedMaterialId;
        } else if (stage === "Assembling") {
          stageMaterialId = productData.assembledMaterialId;
        }

        if (stageMaterialId) {
          const storeRef = doc(db, "rawMaterials", stageMaterialId);
          const storeSnap = await transaction.get(storeRef);

          if (!storeSnap.exists()) {
            throw new Error(
              `Stage store material ${stageMaterialId} not found for product ${resolvedProductDocId}`,
            );
          }
    ...
    // 4) Batch stage counters and materialConsumption map.
    const stagePath = `processingStages.${stage}`;
    const updates: Record<string, any> = {
      [`${stagePath}.accepted`]: increment(acceptedQty),
      [`${stagePath}.rejected`]: increment(rejectedQty),
      [`${stagePath}.lastExecutedAt`]: isoTimestamp,
      [`${stagePath}.lastExecutedBy`]: operatorId,
      updatedAt: isoTimestamp,
    };

    for (const [materialId, consumedQty] of Object.entries(
      consumptionByMaterial,
    )) {
      updates[`${stagePath}.materialConsumptions.${materialId}`] = increment(
        consumedQty,
      );
    }
