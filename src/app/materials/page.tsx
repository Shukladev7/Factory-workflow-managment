...
export default function MaterialsPage() {
  ...
  const filteredAndSortedMaterials = useMemo(() => {
    const filtered = regularMaterials.filter((material) => {
      ...
      const linkedProduct = finalStock.find(
        (product) =>
          Array.isArray(product.bom_per_piece) &&
          product.bom_per_piece.some((row) => row.raw_material_id === material.id),
      )
      const linkedProductId = (linkedProduct?.productId || linkedProduct?.id || "").toLowerCase()
      return (
        material.name.toLowerCase().includes(query) ||
        material.sku.toLowerCase().includes(query) ||
        material.id.toLowerCase().includes(query) ||
        linkedProductId.includes(query)
      )
    })

    return sortArray(filtered, sortDirection, (material) => material.name)
  }, [regularMaterials, finalStock, searchQuery, sortDirection])

  const getProductIdForRawMaterial = (material: RawMaterial) => {
    const linkedProduct = finalStock.find(
      (product) =>
        Array.isArray(product.bom_per_piece) &&
        product.bom_per_piece.some((row) => row.raw_material_id === material.id),
    )
    return linkedProduct?.productId || linkedProduct?.id || "—"
  }
    ...
    <>
      ...
      <div className="mb-4 flex gap-4 items-center">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search by name, SKU, Product ID, or System ID..."
            value={searchQuery}
              ...
              {filteredAndSortedMaterials.length === 0 ? (
                ...
                filteredAndSortedMaterials.map((material: RawMaterial) => (
                  <TableRow key={material.id}>
                    ...
                    <TableCell className="font-mono text-xs">{getProductIdForRawMaterial(material)}</TableCell>
