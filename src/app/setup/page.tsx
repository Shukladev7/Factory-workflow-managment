...
const { units, loading: unitsLoading, createUnit, deleteUnit } = useUnits()
...
const handleAddOrderType = async () => {
  ...
  if (orderTypes?.some((t: OrderType) => t.name.toLowerCase() === newOrderType.trim().toLowerCase())) {
    toast({ variant: "destructive", title: "Error", description: "Order type already exists." })
    return
  }

  try {
    await createOrderType({ name: newOrderType.trim() })
    setNewOrderType("")
    ...
    toast({
      variant: "destructive",
      title: "Error",
      description: e?.message || "Failed to add order type.",
    })
  }
}
...
const handleDeleteOrderType = async (id: string) => {
  try {
    ...
    toast({ title: "Success", description: "Order type deleted." })
  } catch (e: any) {
    toast({
      variant: "destructive",
      title: "Error",
      description: e?.message || "Failed to delete order type.",
    })
  }
}
const handleAddUnit = async () => {
  ...
  try {
    await createUnit({ name: newUnit.trim() })
    setNewUnit("")
    ...
    toast({
      variant: "destructive",
      title: "Error",
      description: e?.message || "Failed to add unit of measure.",
    })
  }
}
...
const handleDeleteUnit = async (id: string) => {
  try {
    ...
    toast({
      variant: "destructive",
      title: "Error",
      description: e?.message || "Failed to delete unit of measure.",
    })
  }
}
  ...
  <Input
    ...
    onKeyDown={(e) => e.key === "Enter" && handleAddOrderType()}
  />
  <Button onClick={handleAddOrderType} disabled={orderTypesLoading}>
    ...
    <TableRow key={t.id}>
      ...
      <TableCell className="text-right">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => handleDeleteOrderType(t.id)}
        >
          <Trash2 className="h-4 w-4" />
        </Button>
      </TableCell>
