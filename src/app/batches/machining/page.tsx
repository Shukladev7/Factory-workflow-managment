...
export default function MachiningPage() {
  ...
  const getProductForMaterial = (material: RawMaterial): FinalStock | null => {
    ...
    const byLink = finalStock.find((p) => p.machinedMaterialId === material.id)
    ...
    const baseName = (material.name || "").replace(/^Machined\s+/i, "").trim()
    if (baseName) {
  ...
  const handleCreateFinalBatch = async (product: FinalStock) => {
    const stage: ProcessingStageName = "Machining"
    if (
  ...
  const allItems = useMemo(() => {
    ...
    const materialItems = finishedMaterials.map((m) => {
    ...
    const finalStage: ProcessingStageName = "Machining"
    const finalStockItems = finalStock
      ? finalStock
          ...
          .map((p) => {
            ...
            const stageThreshold = p.machinedThreshold
  ...
  }, [finishedMaterials, finalStock])

  const handleCreateBatch = async (material: RawMaterial) => {
    const product = getProductForMaterial(material)
    if (!product) {
      toast({ variant: "destructive", title: "No Linked Product", description: "Could not determine the product for this machined material." })
      return
    }
    const stage: ProcessingStageName = "Machining"
  ...
  return (
    <>
      <PageHeader
        ...
        description="Process batches in the machining stage. Log accepted units."
      />
      <Card>
