...
export function EditProductForm({ product, onProductUpdated }: EditProductFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const existingFinalStockIds = new Set(finalStock.map((p) => p.id));
    const validBomRows = bomRows.filter((r) => {
      ...
      if (existingFinalStockIds.has(r.raw_material_id)) {
        return true;
      }

      if (existingMaterialIds.has(r.raw_material_id)) {
        return true;
      }

      // Keep legacy/foreign IDs instead of dropping the row silently.
      return true;
    });
    ...
    const finalStockIds = new Set(finalStock.map((p) => p.id));
    ...
    const sanitizedBomRows = adjustedBomRows.map(({ raw_material_id, stage, qty_per_piece, unit, notes, source }) => {
      ...
      if (source === "final" || source === "raw") {
        row.source = source;
      } else if (finalStockIds.has(raw_material_id)) {
        row.source = "final";
      } else if (rawMaterialIds.has(raw_material_id)) {
        row.source = "raw";
      }
      return row;
    });
    ...
    const updatedProduct: FinalStock = {
      ...
      bom_per_piece: sanitizedBomRows.length > 0 ? sanitizedBomRows : undefined,
    };
