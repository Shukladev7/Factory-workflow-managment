...
export function CreateBatchForm({ onBatchCreated, initialProductId, initialStage }: CreateBatchFormProps) {
    ...
    if (selectedProductId && selectedProcesses.length > 0) {
      ...
      if (
        ...
        const bomMaterials = filteredBOM.map((bomRow) => {
          const isFinal =
            (bomRow as any).source === "final" ||
            finalStock.some((p) => p.id === bomRow.raw_material_id);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // Ensure every selected process has at least one raw material input row
    const stagesMissingMaterials = values.selectedProcesses.filter((stage) => {
      return !values.materials.some((material) => material.stage === stage);
    });

    if (stagesMissingMaterials.length > 0) {
      toast({
        variant: "destructive",
        title: "Missing Raw Material Input",
        description: `Raw material input not defined for stage(s): ${stagesMissingMaterials.join(", ")}. Each selected process must have at least one material row.`,
      });
      setIsSubmitting(false);
      return;
    }
                                  ...
                                  <>
                                    {regularMaterials.map((material) => (
                                      <SelectItem
                                        ...
                                        value={material.id}
                                      >
                                        {formatNameWithId(material.name, material.id)} (In Stock:{" "}
                                    ...
                                    {selectedFinal && !regularMaterials.some((m) => m.id === selectedId) && (
                                      <SelectItem key={selectedId} value={selectedId}>
                                        {formatNameWithId(selectedFinal.name, selectedFinal.id)} (Final Stock)
                                      </SelectItem>
                                    )}
                                  </>
                                )}
                                  ...
                                  {regularMaterials.length === 0 ? (
                                    ...
                                    regularMaterials.map((material) => (
                                      <SelectItem
                                        ...
                                        value={material.id}
                                      >
                                        {formatNameWithId(material.name, material.id)} (Stock:{" "}
                                        {material.quantity.toLocaleString()}{" "}
                                        {material.unit})
                                      </SelectItem>
                                    ))
                                    ...
                                    {assembledMaterials.length === 0 ? (
                                      ...
                                      assembledMaterials.map((material) => (
                                        <SelectItem
                                          ...
                                          value={material.id}
                                        >
                                          🧩 {formatNameWithId(material.name, material.id)} (Stock: {material.quantity.toLocaleString()} {material.unit})
                                        </SelectItem>
                                      ))
                                    )}
                                  </SelectContent>
